...
export class SkillFilter {
  ...
  getGui() {
    ...
    eInstructions.innerHTML = FILTER_TITLE.replace(
      "TITLE_NAME",
    ...
    RefData.IT_SKILLS.forEach(function(skill, index) {
      ...
      let html = SKILL_TEMPLATE.replace("SKILL_NAME", skillName).replace(
        "SKILL",
        skill
      );
  ...
  isFilterActive() {
    ...
    let somethingSelected =
      model.android || model.css || model.html5 || model.mac || model.windows;
