...
def getLive_ticker(identifier):
    try:
        head = {'Accept-Profile': 'public'}
        resp = requests.get(
            'http://185.231.115.223:3000/rpc/liveticker?a='+str(identifier), headers=head)
        if resp.status_code == 200 and not resp.text == []:
            js = json.loads(resp.text)
            if js[0]['ShareCount'] != '-' or js[0]['ShareCount'] != '':
                js[0]['ShareCount'] = float(js[0]['ShareCount'])
            if js[0]['Mabna'] != '-' or js[0]['Mabna'] != '':
                js[0]['Mabna'] = float(js[0]['Mabna'])
...
def AllIndicesImpact():
    head = {'Accept-Profile': 'marketwatch'}
    resp = requests.get(
        'http://185.231.115.223:3000/View_IndicesImpactOnIndex', headers=head)
    if resp.status_code == 200:
        js = json.loads(resp.text)
...
def AllIndicesHH():
    head = {'Accept-Profile': 'marketwatch'}
    resp = requests.get(
        'http://185.231.115.223:3000/View_Industries_HH', headers=head)
    if resp.status_code == 200:
        js = json.loads(resp.text)
...
def getIndicesDetails(identifier):
    ...
    # ************** REQUESTS *****************
    resp = requests.get(
        'http://185.231.115.223:3000/View_Index_Include?ID=eq.'+str(identifier), headers=head)
    technicalResp = requests.get(
        'http://185.231.115.223:3000/View_indices_allstocks_indicators?ID=eq.'+str(identifier), headers=head2)
    ShakhesResp = requests.get(
        'http://185.231.115.223:3000/View_indexValue_Today?ID=eq.'+str(identifier), headers=head)
    # ************** REQUESTS *****************
    ...
    for item in input:
        ...
        if item["close"] and item["yesterday"] != 0:
            item["lastPercent"] = truncater(
                ((item["close"] - item["yesterday"])/item["close"]) * 100)
        else:
            item["lastPercent"] = 0
        ...
        item["HH"] = (item["VolumeBuy_Haghighi"] -
                      item["VolumeSell_Haghighi"]) * item["close"]
...
def DataModifier(input):
    ...
    for item in input:
        ...
        Impact.append(
            {"id": item["firm"], "ticker": item["ticker"], "Impact": item["Impact"]})
        marketCap.append(
            {"id": item["firm"], "ticker": item["ticker"], "marketcap": item["marketcap"]})
        HH.append(
            {"id": item["firm"], "ticker": item["ticker"], "HH": item["HH"]})
        technical.append(
            {"id": item["firm"], "ticker": item["ticker"], "signal": item["signal"]})
        table.append({"id": item["firm"], "ticker": item["ticker"], "last": item["last"],
                     "lastPercent": item["lastPercent"], "marketcap": item["marketcap"]})
...
def ShakhesDataModifier(input):
    ...
    for item in input:
        ...
        RaWdata.append(
            {"HourMinute": item["HourMinute"], "Value": item["Value"], "unix": UTCtimeUNIX})
        apexData.append([UTCtimeUNIX, item["Value"]])
    export = {'CorrectName': item['CorrectName'],
              'RaWdata': RaWdata, "apexData": apexData}
    return export
...
def getLastActiveDayTepix():
    head = {'Accept-Profile': 'marketwatch'}
    resp = requests.get(
        'http://185.231.115.223:3000/View_today_Tepix_reduced', headers=head)
    if resp.status_code == 200:
        js = json.loads(resp.text)
        res = []
        for item in js:
