...
def getBCMarketWatch():
    ...
    while ct < 3:
        resp = requests.get(
            'http://162.55.15.105:3000/View_Crypto_MarketWatch')
        if resp.status_code == 200:
...
def getBCMarketWatchAll():
    ...
    while ct < 3:
        resp = requests.get(
            'http://162.55.15.105:3000/View_Crypto_MarketWatch_All')
        if resp.status_code == 200:
            js = json.loads(resp.text)
            for index, item in enumerate(js):
                item["id"] = index
...
def getSingleCryptoTechnical(firm):
    ...
    while ct < 3:
        resp = requests.get(
            'http://162.55.15.105:3000/View_Crypto_Indicator?coin1=eq.'+str(firm))
        if resp.status_code == 200:
            return (json.loads(resp.text))
            # return(json.loads(resp.text))
        else:
